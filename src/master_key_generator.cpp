...
namespace walletpp {
    ...
    auto master_key_generator::generate_public_key(const extended_key &master_private_extended_key) -> std::unique_ptr<extended_key> {
        ...
        auto extended_master_public_key = std::make_unique<extended_key>(extended_key(std::move(public_key), std::move(copy_of_chaincode), std::move(copy_of_fingerprint), master_private_extended_key.depth,
