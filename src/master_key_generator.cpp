...
namespace walletpp {
    auto master_key_generator::generate_private_key(const secure_vector<uint8_t> &seed) -> extended_key {
    ...
    auto master_key_generator::generate_public_key(const extended_key &master_private_extended_key) -> extended_key {
        ...
        auto extended_master_public_key = extended_key(std::move(public_key), std::move(copy_of_chaincode), std::move(copy_of_fingerprint), master_private_extended_key.depth,
                                                       master_private_extended_key.index);

        return extended_master_public_key;
    }

    auto master_key_generator::generate_master_key_pair(const secure_vector<uint8_t> &seed) -> key_pair {
