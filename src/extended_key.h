...
namespace walletpp {
    struct extended_key {
        ...
        extended_key(secure_vector<uint8_t> &&key, secure_vector<uint8_t> &&chain_code, secure_vector<uint8_t> &&parent_finger_print, const uint8_t depth,
                     const uint64_t index)
            : key(std::move(key)), chain_code(std::move(chain_code)), parent_finger_print(std::move(parent_finger_print)), depth(depth), index(index), is_private_key([this]() -> bool {
